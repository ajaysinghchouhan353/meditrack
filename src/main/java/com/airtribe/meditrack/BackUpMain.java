...
public class BackUpMain {
    ...
    private static void printMainMenu() {
        ...
        System.out.println("5. Demonstrate Design Patterns");
        System.out.println("6. Demonstrate Streams API");
        System.out.println("7. Demonstrate Cloning");
        System.out.println("8. Run Full Demo");
    ...
    private static void demonstratePatterns() {
        ...
        System.out.println("1) Singleton: AppConfig tax rate = "
            + String.format("%.1f%%", AppConfig.getInstance().getTaxRate() * 100));

        System.out.println("2) Observer: Registered listeners = " + notificationService.getListenerCount());

        System.out.println("3) Strategy + Factory: Create sample bill with strategy");
        try {
            Doctor doctor = doctorService.registerDoctor(
                "Dr. Demo", "demo@meditrack.com", "9999999999", "Demo City", "CARDIOLOGY", "LIC-DEMO", 10
            );
            Patient patient = patientService.registerPatient(
                "Demo Patient", "patient@meditrack.com", "8888888888", "Demo City",
                LocalDate.of(1990, 1, 1), "Male", "O+"
            );
            Appointment appointment = appointmentService.bookAppointment(
                doctor.getId(), patient.getId(), LocalDateTime.now().plusDays(1), "Pattern demo"
            );
            Bill bill = billService.createBill(appointment.getId(), 500.0, "PREMIUM");
            System.out.println("Sample premium bill amount = " + String.format("%.2f", bill.getTotalAmount()));
        } catch (Exception ex) {
            System.err.println("Pattern demo failed: " + ex.getMessage());
        }

        System.out.println("4) Enum values - AppointmentStatus");
        for (AppointmentStatus status : AppointmentStatus.values()) {
            System.out.println("   " + status.getCode() + " - " + status.getDescription());
        }
        System.out.println("5) Enum values - Specialization");
        for (Specialization specialization : Specialization.values()) {
            System.out.println("   " + specialization.getName() + " - " + specialization.getDescription());
        }
    }
    ...
    private static void demonstrateStreams() {
        ...
        try {
            ...
            System.out.println("Cardiologists count: " + cardiologists.size());

            List<Appointment> completed = appointmentService.getAppointmentsByStatus(AppointmentStatus.COMPLETED);
            System.out.println("Completed appointments count: " + completed.size());

            List<Bill> paid = billService.getPaidBills();
            double paidRevenue = paid.stream().mapToDouble(Bill::getAmount).sum();
            System.out.println("Paid bills count: " + paid.size());
            System.out.println("Paid bills revenue (stream sum): " + String.format("%.2f", paidRevenue));
        } catch (Exception ex) {
            System.err.println("Streams demo failed: " + ex.getMessage());
        }
    }
    ...
    private static void demonstrateCloning() {
        ...
        try {
            Patient original = new Patient(
                "PAT-DEMO", "Clone Source", "clone@meditrack.com", "7777777777",
                "Clone City", LocalDate.of(1995, 5, 5), "Female", "A+"
            );
            original.setMedicalHistory("Allergy");

            Patient clone = original.clone();

            System.out.println("Original hash: " + System.identityHashCode(original));
            System.out.println("Clone hash:    " + System.identityHashCode(clone));
            System.out.println("Same object?   " + (original == clone));
            System.out.println("Original: " + original);
            System.out.println("Clone:    " + clone);
        } catch (CloneNotSupportedException ex) {
            System.err.println("Clone demo failed: " + ex.getMessage());
        }
    }
